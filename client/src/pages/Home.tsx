...
export default function Home() {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden" style={{ background: '#0a0808' }}>
      ...
      {isInGame && gameState.knowledge.length > 0 && (
        <KnowledgePanel knowledge={gameState.knowledge} />
      )}
      {isInGame && gameState.knowledgeCards.length > 0 && (
        <KnowledgeCardPanel cardIds={gameState.knowledgeCards} />
      )}

      {/* 内容区：预留顶部与左右安全区，避免与章节导航、道具重叠；小屏自适应 */}
      <div
        className={`absolute inset-0 overflow-auto ${isInGame ? 'pt-[5rem] sm:pt-24 pl-[max(1rem,calc(0.75rem+45vw))] sm:pl-52 pr-[5rem] sm:pr-28' : ''}`}
      >
