...
export default function Chapter3Scene(props: Chapter3SceneProps) {
  ...
  // 小屏自适应：对 460px 球圈整体缩放（px-6 两侧各24px，减去48px后得可用宽）
  const [circleScale, setCircleScale] = useState(1);
  useEffect(() => {
    const update = () => {
      const available = window.innerWidth - 48;
      setCircleScale(available < 460 ? Math.max(0.5, available / 460) : 1);
    };
    update();
  ...
  return (
    <div className="fixed inset-0 flex flex-col">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-0 flex-1 overflow-y-auto px-6 py-6">
        {/* Chapter header */}
          ...
          {subPhase === 'resonance-q' && (
            ...
            >
              ...
              {!resonanceShowFeedback ? (
                <div className="relative w-full overflow-visible mb-6" style={{ height: 460 * circleScale }}>
                  <div className="absolute overflow-visible" style={{ width: 460, height: 460, top: '50%', left: '50%', transform: `translate(-50%, -50%) scale(${circleScale})`, transformOrigin: 'center center' }}>
